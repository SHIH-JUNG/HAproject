...
$(document).ready(function () {
  $.ajax({
    url: "database/find_members_assemble_data_detail.php",
    data: {
      ma_id: ma_id,
      year: ma_year,
    },
        ...
        if (inputs_type_files.length > 0) {
          ...
          for (var i = 0; i < inputs_type_files.length; i++) {
            ...
            var file_html =
              '<a name="customFile' +
              (i + 1) +
              '_a" href="./members_assemble/upload/' +
              file_val +
              '" style="text-decoration:none;color:blue;" target="_blank">' +
              file_val +
              '<br/><img style="vertical-align:middle;" width="auto" src="./members_assemble/upload/' +
              file_val +
              '"></a>';
...
function jsignature_initialization(init_name) {
  ...
  $("#"+init_name+"_signature_submit").click(function(){
      ...
      var ajax_url = 'database/update_members_assemble_data_detail_signature.php';
...
function rec_update_fillin() {
  ...
  if (check_updat_members_assemble_data() != "") {
    stau = false;
  } else {
    stau = true;
  }
  ...
  if (!stau) {
    swal({
      title: check_updat_members_assemble_data(),
      type: "error",
    });
  } else {
    $.ajax({
      url: "database/update_members_assemble_data_detail.php",
      type: "POST",
      data: {
        ma_id: ma_id,
        record_content: form,
        ...
        if (data == 1) {
          swal({
            ...
            window.location.href =
              "members_assemble_detail.php?year=" +
...
function submit_form_data_upload() {
  ...
  $.ajax({
    url: "database/update_upload_members_assemble_data_detail.php",
    type: "POST",
    data: form_data,
    contentType: false,
    cache: false,
...
function check_updat_members_assemble_data() {
