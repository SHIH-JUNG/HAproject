...
$new_file = "../members_assemble/signature/";  
...
if (file_put_contents($new_file,base64_decode($base64_content[1]))){
	$sqlUpdate ="UPDATE `members_assemble_v2` SET $sql_str WHERE `Id` = '$ma_id' ORDER BY `members_assemble_v2`.`Create_date` ASC LIMIT 1;";

    $sqlUpdate .="UPDATE `signature_notice` SET `Sign_state` = REPLACE(`Sign_state`, '$sign_state_sqlstr1', '$sign_state_sqlstr2') WHERE `Sign_id` = '$ma_id' AND `Type`='members_assemble' ORDER BY `signature_notice`.`Id` ASC LIMIT 1;";
