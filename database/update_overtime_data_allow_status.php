...
if($row_nums > 0)
{
    ...
    if($Allow_status == "核准")
    {
        $select_overtime_data = "SELECT `Resume_id`, `Rec_year`, `Overtime_hours`, `Free_hours`, `Free_date`, `Overtime_date` FROM `overtime` WHERE `Id` = '$Overtime_id';";
        ...
        $Remark = "新增的補休時數：" . number_format(floatval($overtime_data[2]), 1) . "小時，加班日期：" . $overtime_data[5] . "。使用的補休時數：" . number_format(floatval($overtime_data[3]), 1) . "小時，補休日期：" . $overtime_data[4] . "。";
        $overtime_data_2f = number_format(floatval($overtime_data[2]) - floatval($overtime_data[3]), 1);
