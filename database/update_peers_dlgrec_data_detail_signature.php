...
if (file_put_contents($new_file,base64_decode($base64_content[1]))){
    if($user == $sign_name){
        $sqlUpdate ="UPDATE `peers_dlgrec` SET $sql_str WHERE `Id` = '$peers_dlgrec_id' ORDER BY `peers_dlgrec`.`Create_date` ASC LIMIT 1;";

        $sqlUpdate .="UPDATE `signature_notice` SET `Sign_state` = REPLACE(`Sign_state`, '$sign_state_sqlstr1', '$sign_state_sqlstr2') WHERE `Sign_id` = '$peers_dlgrec_id' AND `Type`='peers_dlgrec' ORDER BY `signature_notice`.`Id` ASC LIMIT 1;";
