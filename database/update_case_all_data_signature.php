...
if (file_put_contents($new_file, base64_decode($base64_content[1]))) {
    if($user == $sign_name){
        $sqlUpdate = "UPDATE `form_all_info` SET $sql_str WHERE `Id` = '$sign_info_arr[0]' AND `Case_id` = '$sign_info_arr[2]' AND `Form_name` = '$submit_board_type' LIMIT 1;";
        $sqlUpdate .="UPDATE `signature_notice` SET `Sign_state` = REPLACE(`Sign_state`, '$sign_state_sqlstr1', '$sign_state_sqlstr2') WHERE `Sign_id` = '$sign_info' AND `Type`='current_case' ORDER BY `signature_notice`.`Id` ASC LIMIT 1;";
