...
$note = "SELECT *, SUM(ROUND(`Income_sum`,0)) AS Income_sum, SUM(ROUND(`Cost_sum`,0)) AS Cost_sum, SUM(ROUND(`Last_pb`,0)) AS Last_pb, SUM(ROUND((`accounting_record_cash_balance_v2`.`Income_sum`-`accounting_record_cash_balance_v2`.`Cost_sum`), 0)) AS This_pb FROM `accounting_record_cash_balance_v2` Where `Year` = '$year' ORDER BY `accounting_record_cash_balance_v2`.`Id` ASC;";
