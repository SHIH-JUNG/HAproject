...
switch ($sign_type) {
    ...
    case 'supervise':
        ...
        break;
    case 'social_worker':
        $sql_str = " `Ssocial_worker` = '$user', `Ssocial_worker_signature` = '$new_file', `Social_worker_sign_msg` = '$sign_msg',`Social_worker_sign_time` = NOW()";
        break;
    default:
        return false;
        break;
...
if (file_put_contents($new_file, base64_decode($base64_content[1]))) {
    if($user == $sign_name){
        $sqlUpdate = "UPDATE `volunteer_v2` SET $sql_str WHERE `Id` = '$vo_id' AND `Year`='$year_num' ORDER BY `volunteer_v2`.`Create_date` ASC LIMIT 1;";

        $sqlUpdate .="UPDATE `signature_notice` SET `Sign_state` = REPLACE(`Sign_state`, '$sign_state_sqlstr1', '$sign_state_sqlstr2') WHERE `Sign_id` = '$vo_id' AND `Type`='volunteer' ORDER BY `signature_notice`.`Id` ASC LIMIT 1;";
