...
if($form_id_num[0]>0)
{
    ...
    if(isset($_FILES["file4"]))
    {
        ...
        if($_FILES["file4"]["name"] != null){
            ...
            WHERE `Case_id` = '$Case_id' AND `Form_id` = '$Form_id' AND `Form_type` = '$Form_type' AND `Case_pid` = '$Case_pid';";
            ...
            {
                $select_case_storage = "SELECT `Case_storage` FROM `current_case` WHERE `Case_id` = '$Case_id' AND `Id` = '$Case_seqid';";
...
else // 第一次新增至 forms 資料庫，執行insert
{
    // 判斷有無上傳檔案
