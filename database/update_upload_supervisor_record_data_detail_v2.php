...
if (isset($_FILES["agenda_files"]))
{
    ...
    // 查詢 原本的`File_path`
    $select_0_file_num = "SELECT `Agenda_file_path` FROM `supervisor_record_v2` WHERE `Id` = '$sr_id' ORDER BY `supervisor_record_v2`.`Create_date` ASC;";
    ...
    // 查詢 會議上傳提醒
    $select_RW_num = "SELECT `Id`, `Record_id`, `R_or_G`, `State` FROM `record_warn` WHERE `Record_id` = '$sr_id' AND `Rtype_name` = 'supervisor_record' AND `R_or_G` = 'G';";
...
if (isset($_FILES["rec_files"]))
{
    ...
    // 查詢 原本的`File_path`
    $select_1_file_num = "SELECT `Rec_file_path` FROM `supervisor_record_v2` WHERE `Id` = '$sr_id' ORDER BY `supervisor_record_v2`.`Create_date` ASC;";
    ...
    // 查詢 會議上傳提醒
    $select_RW_num = "SELECT `Id`, `Record_id`, `R_or_G`, `State` FROM `record_warn` WHERE `Record_id` = '$sr_id' AND `Rtype_name` = 'supervisor_record' AND `R_or_G` = 'R';";
...
$sqlUpdate = "UPDATE `supervisor_record_v2` SET `upload_content` = '$upload_content' ". $file_sql  ."
