...
$.ajax({
    ...
    success: function (data) {
        var board_cssString1 = "";
        var board_cssString2 = "";
        ...
        //存放sql抓到的板子類型
        var sign_board_type_sql1= [];
        var sign_board_type_sql2 = [];
        ...
        //     console.log(sign_state_arr)
            ...
            switch (panel_color) {
                case "#FF9797":
                        sign_board_type_sql1.push(value.Type);
                    break;
                case "#90ee90":
                        sign_board_type_sql2.push(value.Type);
                    break;
                default:
                        sign_board_type_sql2.push(value.Type);
                    break;
        ...
        // 比對後 篩選沒有的板子類型
        var sign_board_type_filter1 = sign_board_type.filter(element => sign_board_type_sql1.includes(element));
        var sign_board_type_filter2 = sign_board_type.filter(element => sign_board_type_sql2.includes(element));


        //如果是第一個板子 顯示展開
        $.each(sign_board_type_filter1,function(i,v){
            ...
            '<div class="panel-heading" role="tab" id="panel_1_heading_' + v + '">' +
            '<h4 class="panel-title">' +
                '<a role="button" data-toggle="collapse" data-parent="#accordion_panel_1" href="#panel_1_collapse_' + v + '" aria-expanded="true" aria-controls="panel_1_collapse_' + v + '" style="font-size: large;">' +
                find_board_big5_str +
                
                '</a>' +
            '</h4>' +
            '</div>' +
            '<div id="panel_1_collapse_' + v + '" class="panel-collapse ' + collapse_state + '" role="tabpanel" aria-labelledby="panel_1_heading_' + v + '">' +
        ...
        $("#accordion_panel_1").append(board_cssString1);


        //如果是第一個板子 顯示展開
        $.each(sign_board_type_filter2,function(i,v){
            ...
            board_cssString2 += 
            '<div class="panel panel-warning" style="padding-bottom: .5em;">' +
            '<div class="panel-heading" role="tab" id="panel_2_heading_' + v + '">' +
            '<h4 class="panel-title">' +
                '<a role="button" data-toggle="collapse" data-parent="#accordion_panel_2" href="#panel_2_collapse_' + v + '" aria-expanded="true" aria-controls="panel_2_collapse_' + v + '" style="font-size: large;">' +
                find_board_big5_str +
                
                '</a>' +
            '</h4>' +
            '</div>' +
            '<div id="panel_2_collapse_' + v + '" class="panel-collapse ' + collapse_state + '" role="tabpanel" aria-labelledby="panel_2_heading_' + v + '">' +
            '<div class="panel-body">' +
                '<table style="font-size:15px;font-family:新細明體;" class="text-center table-hover table-striped table-sm" data-toggle="table" data- data-page-size=5 data-search="false" data-pagination="true" data-pagination-parts="[pageList]">' +
                    '<thead>' +
                    '<tr>' +
    ...
    success: function (data) {
        ...
        $.each(data,function(index,value){
           ...
           console.log(signer_arr)
            ...
            if (panel_color == "#FF9797") 
            {
                $("#panel_1_" + value.Type + "_tb").append(cssString);    
            } 
            else
            {
                $("#panel_2_" + value.Type + "_tb").append(cssString);    
            }
