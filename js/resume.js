  ...
  success: function (data) {
    ...
    $.each(data, function (index, value) {
      cssString +=
        ...
        "</td>" +
        '<td style="text-align:center">' +
        check_sql_date_format(value.Upload_date) +
        "</td>" +
        '<td style="text-align:center">' +
        check_sql_date_format(value.Entry_date) +
        "</td>" +
        '<td style="text-align:center">' +
        value.On_or_off +
        "</td>" +
    ...
    $(".table-hover tbody").on("click", "tr", function () {
      window.location.href = "resume_detail.php?id=" + $(this).attr("id") + "";
    });
  },
...
var upload_date_range = function (settings, data, dataIndex) {
  var min_date = parseInt(
    Date.parse(trans_to_EN($("#upload_min_date").val())),
    10
  );
  var max_date = parseInt(
    Date.parse(trans_to_EN($("#upload_max_date").val())),
    10
  );
...
var entry_date_range = function (settings, data, dataIndex) {
  var min_date = parseInt(
    Date.parse(trans_to_EN($("#entry_min_date").val())),
    10
  );
  var max_date = parseInt(
    Date.parse(trans_to_EN($("#entry_max_date").val())),
    10
  );
  var date = parseInt(Date.parse(trans_to_EN(data[4]))) || 0; // use data for the date column
  if (
...
var HIV_date_range = function (settings, data, dataIndex) {
  var min_date = parseInt(
    Date.parse(trans_to_EN($("#HIV_min_date").val())),
    10
  );
  var max_date = parseInt(
    Date.parse(trans_to_EN($("#HIV_max_date").val())),
    10
  );
...
var out_prison_date_range = function (settings, data, dataIndex) {
  var min_date = parseInt(
    Date.parse(trans_to_EN($("#out_prison_min_date").val())),
    10
  );
  var max_date = parseInt(
    Date.parse(trans_to_EN($("#out_prison_max_date").val())),
    10
  );
...
var in_prison_date_range = function (settings, data, dataIndex) {
  var min_date = parseInt(
    Date.parse(trans_to_EN($("#in_prison_min_date").val())),
    10
  );
  var max_date = parseInt(
    Date.parse(trans_to_EN($("#in_prison_max_date").val())),
    10
  );
