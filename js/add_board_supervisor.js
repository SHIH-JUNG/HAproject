...
$("#rec_add_new").on("click", function () {
  ...
  var meeting_date_year_split = $("#meeting_date").val().split("年");
  var title = '理監事會議記錄簽核：'+$("#title_name").val();
  // console.log(form)
  ...
  if (!stau) {
    ...
    $.ajax({
      url: "database/add_new_board_supervisor.php",
      type: "POST",
      data: {
        record_content: form,
        ...
        if (data == 1) {
          swal({
            ...
            window.location.href =
              "board_supervisor.php?year=" + meeting_date_year_split[0];
          });
        } else {
          swal({
            type: "error",
...
function submit_form_data_upload() {
  ...
  form_data.append("title", '理監事會議記錄簽核：'+$("#upload_title_name").val());
  ...
  $.ajax({
    url: "database/add_new_upload_board_supervisor.php",
    type: "POST",
    data: form_data,
    contentType: false,
    cache: false,
    ...
    success: function (data) {
      ...
      if (data == 1) {
        swal({
          ...
          window.location.href =
            "board_supervisor.php?year=" + upload_rec_date_year_split[0];
        });
      } else {
        swal({
          type: "error",
