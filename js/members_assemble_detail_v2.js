...
signature_submit = function(this_btn) {
  ...
  var ajax_url = "database/update_members_assemble_data_detail_signature_v2.php";
...
{
  $.ajax({
    url: "database/find_members_assemble_data_detail_v2.php",
    data: {
      ma_id: ma_id,
      year: ma_year,
    },
      ...
      $.each(data, function (index, value) {
        ...
        if(value.Agenda_file_path != "")
        {
          $.each(customFile1_arr, function (i, val) {
            ...
            {
              customFile1_htmlstr += '<input class="ma_question" style="zoom: 1.5" class="form-check-input" type="radio" name="customFile1_check" file_type_name="agenda_file_path" forms_sql_id="' + value.Id + '" value="' + i + '">'
        ...
        customFile1_htmlstr += '<br/>'
          + '<button class="ma_question" style="color:red;margin-right:3em;margin-bottom:.5em;" type="button" onclick="selectFiles_delete(0);">刪除</button>'
          + '<div>※點選上面要刪除的檔案</div>'
          + '<br/><hr style="border:3px dashed blue; height:1px">'
          + '<button class="ma_question" style="color:blue;" type="button" onclick="selectFiles_insert(0);">新增檔案+</button><br/><div id="selected-files1"><span style="color:red;">上傳檔案清單預覽：</span><br/></div>';
        ...
        if(value.Rec_file_path != "")
        {
          $.each(customFile2_arr, function (i, val) {
            ...
            {
              customFile2_htmlstr += '<input class="ma_question" style="zoom: 1.5" class="form-check-input" type="radio" name="customFile2_check" file_type_name="rec_file_path" forms_sql_id="' + value.Id + '" value="' + i + '">'
        ...
        customFile2_htmlstr += '<br/>'
          + '<button class="ma_question" style="color:red;margin-right:3em;margin-bottom:.5em;" type="button" onclick="selectFiles_delete(1);">刪除</button>'
          + '<div>※點選上面要刪除的檔案</div>'
          + '<br/><hr style="border:3px dashed blue; height:1px">'
          + '<button class="ma_question" style="color:blue;" type="button" onclick="selectFiles_insert(1);">新增檔案+</button><br/><div id="selected-files2"><span style="color:red;">上傳檔案清單預覽：</span><br/></div>';

        $("#customFile2").html(customFile2_htmlstr);
        //customFile1顯示資料處理 endregion

        //supervise顯示簽名 region
        var supervise_sign_file_val = value.Supervise_signature.replace("\.\.\/members_assemble\/signature\/", "");
        ...
        {
          $("#supervise_signature_simg").attr("href", "./members_assemble/signature/"+supervise_sign_file_val)
        }
        ...
        //director顯示簽名 region
        var director_sign_file_val = value.Director_signature.replace("\.\.\/members_assemble\/signature\/", "");
        ...
        {
          $("#director_signature_simg").attr("href", "./members_assemble/signature/"+director_sign_file_val)
        }
  ...
  $(".ma_question").prop("disabled", true);
}
...
function submit_form_data_upload() 
{
  ...
  $.ajax({
    url: "database/update_upload_members_assemble_data_detail_v2.php",
    type: "POST",
    data: form_data,
    contentType: false,
    cache: false,
  ...
  if ($("[name='"+file_type_n+"_check']:checked").length > 0) {
    ...
    swal({
      ...
      if (result) {
        ...
        var ma_file = file_input_val_arr.splice(parseInt(file_val), 1);
        // console.log("------------------------")
        // console.log(file_sql_id)
        // console.log(ma_id)
        // console.log(file_input_val_arr)
        // console.log(file_val)
        // console.log(ma_file[0])

        $.ajax({
          url: "database/delete_ma_file.php",
          type: "POST",
          data: {
...
//會議記錄總表格鎖定控制region
function ma_edit() {
  $(".ma_question").attr("disabled", false);
...
$("#preview_word").on("click", function () {
  var ma_id = getUrlVars()["ma_id"];
  //    console.log(id);
  location.href = "preview_word.php?ma_id=" + ma_id + "";
});

$("#preview_word2").on("click", function () {
  var ma_id = getUrlVars()["ma_id"];
  //    console.log(id);
  location.href = "preview_word2.php?ma_id=" + ma_id + "";
});
//endregion
