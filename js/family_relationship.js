    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var index in data.face_id) {
                cssString += 
                        '<tr id="'+data.id[index]+'" face_id="'+data.face_id[index]+'" name="'+data.name[index]+'" addition="'+data.addition[index]+'">' +
                            '<td style="text-align:center">' + data.face_id[index] + '</td>' +
                            ...
                            '<td style="text-align:center">' + data.address[index] + '</td>' + 
                        '</tr>'
                $("#face_id").append('<option value="'+data.face_id[index]+'">'+data.face_id[index]+'</option>');
            } 
                $("#face_id").children().each(function() {
                    text = $(this).text();
                    if($("select#face_id option:contains("+text+")").length > 1){
                        $("select#face_id option:contains("+text+"):gt(0)").remove();
                    }
//                    console.log(text)
                });

            //印出表格
            $("#family_view").html(cssString);
            
            //點擊table tr 進入詳細頁面
            $(".table-hover tbody").on("click","tr",function () {
                window.location.href = 'family_all.php?id='+$(this).attr("id")+'&face_id='+$(this).attr("face_id")+'&name='+$(this).attr("name")+'&addition='+$(this).attr("addition")+'';
            });
        },
...
//table設定region
var $table = $('#family_all').DataTable({
    "ordering": false,
    "info": true,
    "paging": true,
    "lengthChange": false,
