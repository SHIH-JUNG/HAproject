...
//                        '<span name="user'+index+'[]" class="user" id="user'+index+'"></span>'
        ...
        if (data.abstinence[0] !== "") {
            $('input[name=abstinence][value=' + data.abstinence[0] + ']').attr('checked', true);
        }
        if (data.care_who[0] !== "") {
            $('input[name=care_who][value=' + data.care_who[0] + ']').attr('checked', true);
        }
        ...
        if (data.why_not_care[0] !== "" && data.why_not_care[0] !== exp) {

            if (data.why_not_care[0] == "個案失聯" || data.why_not_care[0] == "個案死亡" || data.why_not_care[0] == "家屬拒絕聯繫" || data.why_not_care[0] == "其他") {
                $('input[name=why_not_care][value=' + data.why_not_care[0] + ']').attr('checked', true);
            } else {
                $('input[name=why_not_care][value=其他]').attr('checked', true);
                $("#why_not_care_other").val(data.why_not_care[0]);
            }
        ...
        $("#face_times").val(data.face_times[0]);
        $("#phone_times").val(data.phone_times[0]);
...
//    console.log(type)
    ...
    var abstinence = $("input[name='abstinence']:checked").val();
    ...
    var why_not_care = $("input[name='why_not_care']:checked").val();
    var num = getUrlVars()['number'];
    var family_id = getUrlVars()['family_id'];
...
//    console.log(department,user,suggest)
    //endregion
    ...
    $.ajax({
        url: "database/add_family_note.php",
        type: "POST",
        data: {
            ...
            face_times:$("#face_times").val(),
            phone_times:$("#phone_times").val(),
...
$("#family_preview").on('click',function(){
    var face_id = getUrlVars()["face_id"];
    var number = getUrlVars()["number"];
    var family_id = getUrlVars()["family_id"];
//    console.log(id);
    location.href = 'family_preview.php?face_id='+face_id+'&number='+number+'&family_id='+family_id+'';
});
